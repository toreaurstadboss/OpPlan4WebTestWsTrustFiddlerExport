...
namespace Fiddler.WebTesting
{
    public class FiddlerWebTest
    {
        ...
        {
            this.Save(Path, evtProgressNotifications, true, new frmSelectPlugins(new List<PluginClassReference>()));
        }
        ...
        private static string GetRequestBodyAsString(Session oSession)
        {
            try
            {
                ISamlTokenParameterizer samlTokenParameterizer = new SamlTokenParameterizer();
                string requestBody = oSession.GetRequestBodyAsString();
                string samlParameterizedSoapEnvelope = samlTokenParameterizer.TokenizeSoapMessage(requestBody);
                return samlParameterizedSoapEnvelope;
            }
            catch (Exception err)
            {
                return err.ToString();
